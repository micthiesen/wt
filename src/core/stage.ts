...
function truncateSlug(s: string, limit = SLUG_MAX_LEN): string {
...
export function dirSlug(branch: string): string {
  ...
  if (branch.startsWith(`${BRANCH_PREFIX}/`)) {
    return truncateSlug(branch.slice(BRANCH_PREFIX.length + 1).replace(/\//g, "-"));
  }
...
export function computeStage(slug: string): string {
  ...
  if (m) return `${STAGE_PREFIX}${m[1]}-${digest.slice(0, 6)}`;
  return `${STAGE_PREFIX}${digest.slice(0, 10)}`;
}

export function stageUrl(stage: string): string | null {
  if (!STAGE_DOMAIN) return null;
  return `https://${stage}.${STAGE_DOMAIN}/`;
}
