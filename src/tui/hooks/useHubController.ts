...
export function useHubController(opts: HubControllerOpts) {
  ...
  // a session pane you weren't in). Ops ride the same `focusOpRef`
  // queue the flows use, so a picker commit's own focus-session op
  // (enqueued later, when its switch completes) still lands after the
  // restore instead of racing it. Transition-only: acting on the mount
  // value would immediately unfocus the task pane the startup layout
  ...
  useEffect(() => {
    ...
    if (inputActive) {
      ...
      queue(() => (restore ? focusLeft() : focusRight()));
