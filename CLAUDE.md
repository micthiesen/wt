...
- **Sources** — `src/state/queries.ts`, `src/state/hooks.ts`, `src/tui/hooks/useWorktreeRows.ts`. They own fetching, batching, caching via TanStack Query. Small fixed set (github, git, sst, claude, linear-derived); not user-pluggable.
...
- **Driver** — `src/tui/panels/details.tsx`. Iterates the configured row list, computes the trailing staleness glyph, and renders the inline error message verbatim once retries are exhausted.
