...
- **Source fetches stay batched.** The github source is one GraphQL round-trip aliasing every per-worktree PR field (state, checks, reviews, requested + suggested reviewers). Never split it into per-row fetches; rate limits and latency are real. New PR fields go into `PR_FRAGMENT` in `core/github.ts` rather than getting their own query. Mergeability/merge-queue state is NOT here — it comes from a separate `graphiteQuery` (`core/graphite-api.ts`); GitHub's native merge-queue + auto-merge plumbing was dropped in favor of Graphite.
