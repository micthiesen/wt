...
The loader prints every missing or malformed field at once. See [`src/core/config.ts`](src/core/config.ts) for the full schema and defaults, including the row-ordering knob (`[ui] rows`) and the `[[actions]]` tables that populate the `!` action menu.
